...
export function SubmissionStatusScreen({ route, navigation }: Props) {
  ...
  if (query.isError || !query.data) {
    return (
      <View style={styles.centeredContainer}>
        <Text style={styles.errorText}>{t('submissionStatus.loadError')}</Text>
      </View>
    );
  }

  const contribution = query.data;
  const steps = buildTimelineSteps(t, contribution.status);
  const kindLabelKey = CONTENT_KIND_LABEL_KEYS[contribution.type];

  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Text style={styles.title}>
        {kindLabelKey ? t(kindLabelKey) : t('submissionStatus.fallbackTitle')}
      ...
      {contribution.aiReason && (contribution.status === 'rejected' || contribution.status === 'edit_requested') ? (
        <View style={styles.reasonBox}>
          <Text style={styles.reasonLabel}>{t('submissionStatus.reasonLabel')}</Text>
      ...
      {contribution.status === 'edit_requested' ? (
        ...
        >
          <Text style={styles.resubmitButtonText}>
            {contribution.type === 'new_restaurant' ? t('submissionStatus.resubmitRestaurant') : t('submissionStatus.resubmitOther')}
          </Text>
        </Pressable>
      ) : null}
    </ScrollView>
...
function buildTimelineSteps(t: TFunction, status: string): TimelineStep[] {
  const steps: TimelineStep[] = [
    { label: t('submissionStatus.stepSubmitted'), done: true },
    { label: t('submissionStatus.stepProcessing'), done: true },
  ];
  switch (status) {
    case 'pending':
    case 'in_review':
      steps.push({ label: t('submissionStatus.stepPending'), done: true });
      break;
    case 'auto_approved':
    case 'approved':
      steps.push({ label: t('submissionStatus.stepApproved'), done: true });
      break;
    case 'rejected':
      steps.push({ label: t('submissionStatus.stepRejected'), done: true, isNegative: true });
      break;
    case 'edit_requested':
      steps.push({ label: t('submissionStatus.stepEditRequested'), done: true, isNegative: true });
      break;
  }
  return steps;
}
...
const createStyles = (colors: ThemeColors) =>
  StyleSheet.create({
    ...
    reasonBox: { backgroundColor: colors.surfaceAlt, borderRadius: 12, padding: 12, marginTop: 8 },
    ...
    resubmitButton: { backgroundColor: colors.primary, borderRadius: 14, paddingVertical: 14, alignItems: 'center', marginTop: 24 },
