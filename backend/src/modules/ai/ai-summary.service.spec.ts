...
function buildService(opts: {
  ...
  const prisma = {
    restaurantStatus: {
      findUnique: jest
        .fn()
        .mockResolvedValue({ reviewCount: opts.reviewCount }),
    },
    aISummary: {
  ...
  return {
    service: new AiSummaryService(prisma, config, claudeGateway),
    upsertMock,
    claudeGateway,
  };
}
...
describe('AiSummaryService.regenerateIfNeeded', () => {
  it('skips (no Claude call, no upsert) below the minimum review count', async () => {
    const { service, upsertMock, claudeGateway } = buildService({
      reviewCount: 2,
      commentedReviewCount: 2,
    });
    await service.regenerateIfNeeded('r1');
    expect(upsertMock).not.toHaveBeenCalled();
    expect(
      (claudeGateway as { summarize: jest.Mock }).summarize,
