...
export class MediaService {
  ...
  async createUploadUrl(
    ...
    if (!ALLOWED_CONTENT_TYPES.includes(contentType as UploadableImageContentType)) {
      throw new BadRequestException('Định dạng ảnh không được hỗ trợ (chỉ JPEG/PNG/WEBP).');
    }
    ...
    const uploadUrl = await this.s3.presignPut(storageKey, contentType, UPLOAD_URL_EXPIRES_SECONDS);
    const expiresAt = new Date(Date.now() + UPLOAD_URL_EXPIRES_SECONDS * 1000).toISOString();
  ...
  async confirm(userId: string, input: ConfirmUploadInput): Promise<PhotoDto> {
    ...
    await this.s3.deleteObject(input.storageKey).catch((error) => {
      this.logger.warn(`Failed to delete staging object ${input.storageKey}: ${String(error)}`);
    });
  ...
  async remove(userId: string, photoId: string, isAdmin: boolean): Promise<void> {
    ...
    await this.s3.deleteObject(photo.storageKey).catch((error) => {
      this.logger.warn(`Failed to delete ${photo.storageKey} from storage: ${String(error)}`);
    });
    if (photo.thumbnailKey) {
      await this.s3.deleteObject(photo.thumbnailKey).catch((error) => {
        this.logger.warn(`Failed to delete ${photo.thumbnailKey} from storage: ${String(error)}`);
      });
    }
  }
   ...
   * attaching photos.
   */
  async reparent(userId: string, photoIds: string[], ownerType: MediaOwnerType, ownerId: string): Promise<void> {
    if (photoIds.length === 0) return;

    await this.prisma.$transaction(async (tx) => {
      await this.assertUnderCap(ownerType, ownerId, photoIds.length, tx);

      const result = await tx.photo.updateMany({
        where: { id: { in: photoIds }, uploadedBy: userId, ownerId: null, ownerType },
        data: { ownerId },
      });
      if (result.count !== photoIds.length) {
        throw new BadRequestException('Một số ảnh không hợp lệ hoặc đã được gắn vào nơi khác.');
      }
    });
  }
  ...
  async sweepOrphans(): Promise<number> {
    ...
    if (orphans.length > 0) {
      await this.prisma.photo.deleteMany({ where: { id: { in: orphans.map((p) => p.id) } } });
  ...
  private async assertUnderCap(
    ...
    const existingCount = await tx.photo.count({ where: { ownerType, ownerId, deletedAt: null } });
    if (existingCount + incomingCount > cap) {
      throw new BadRequestException(`Chỉ được phép tối đa ${cap} ảnh.`);
    }
  }

  private toDto(photo: { id: string; width: number | null; height: number | null }, storageKey: string): PhotoDto {
    return { id: photo.id, url: this.s3.publicUrl(storageKey), width: photo.width, height: photo.height };
  }
