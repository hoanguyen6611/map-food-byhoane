...
describe('recommendActionForRiskScore', () => {
  it('recommends auto_approve below the medium-risk threshold', () => {
    expect(recommendActionForRiskScore(MEDIUM_RISK_THRESHOLD - 0.01)).toBe(
      'auto_approve',
    );
  });

  it('recommends hold_for_review at or above the medium-risk threshold', () => {
    expect(recommendActionForRiskScore(MEDIUM_RISK_THRESHOLD)).toBe(
      'hold_for_review',
    );
...
describe('scoreTextContentRuleBased', () => {
  it('scores clean text as zero risk with no labels', () => {
    const result = scoreTextContentRuleBased(
      'Quán này rất ngon, phục vụ nhiệt tình.',
    );
  ...
  it('flags a URL', () => {
    const result = scoreTextContentRuleBased(
      'Ghé xem thêm tại https://spam.example.com nhé',
    );
  ...
  it('flags a known spam phrase', () => {
    const result = scoreTextContentRuleBased(
      'Kiếm tiền online dễ dàng, inbox zalo ngay',
    );
  ...
  it('flags all-caps text past the length threshold', () => {
    const result = scoreTextContentRuleBased(
      'QUAN NAY THUC SU RAT LA TUYET VOI CHO MOI NGUOI GHE THAM NHE',
    );
  ...
  it('accumulates multiple signals and clamps at 1.0', () => {
    const result = scoreTextContentRuleBased(
      'CLICK VAO LINK https://x.com KIEM TIEN ONLINE NGAYYYYY',
    );
