...
describe('Admin Moderation Queue (e2e)', () => {
  ...
  async function registerElevated(
    label: string,
    roleCode: 'admin' | 'moderator',
  ...
  async function submitSpammyReview(
    token: string,
  ): Promise<{ reviewId: string; moderationResultId: string }> {
    const res = await request(app.getHttpServer())
      .post('/reviews')
    ...
    const moderationResult = await prisma.moderationResult.findFirstOrThrow({
  ...
  it('lists a flagged review with risk score, labels, reason, and submitter name', async () => {
    ...
    const { token: modToken } = await registerElevated(
      'queue-review-mod',
      'moderator',
    );
  ...
  it('blocks reject/edit_requested without a reason (server-side), both for moderator and admin', async () => {
    ...
    const { reviewId, moderationResultId } =
      await submitSpammyReview(authorToken);
  ...
  it('a moderator can reject with a reason — updates review status, writes AuditLog, creates a Notification for the author', async () => {
    ...
    const { token: modToken, userId: modId } = await registerElevated(
      'queue-reject-mod',
      'moderator',
    );
    const { reviewId, moderationResultId } =
    ...
    const auditRow = await prisma.auditLog.findFirst({
      where: { action: 'moderation.rejected', targetId: reviewId },
    });
    ...
    expect(
      notifications.items.some((n) =>
        n.payload.body.includes('quảng cáo trái phép'),
      ),
    ).toBe(true);
  ...
  it('both admin and moderator can decide (not admin-only, unlike restaurant hard-delete)', async () => {
    ...
    const { token: adminToken } = await registerElevated(
      'queue-bothroles-admin',
      'admin',
    );
    const { reviewId, moderationResultId } =
      await submitSpammyReview(authorToken);
  ...
  it('approving a held contribution publishes the restaurant (ContributionFinalizeService dispatch)', async () => {
    const { token: submitterToken, userId: submitterId } = await registerUser(
      'queue-contrib-submitter',
    );
    const { token: modToken } = await registerElevated(
      'queue-contrib-mod',
      'moderator',
    ...
    const createRes = await request(app.getHttpServer())
      ...
      .send({
        ...
        description:
          'kiếm tiền online dễ dàng, xem tại https://spam.example.com nhé', // force hold_for_review
        address: {
          line: '1 Queue St',
          ward: 'Phường Test',
          province: 'TP. Test',
    ...
    const moderationResult = await prisma.moderationResult.findFirstOrThrow({
      where: { targetType: 'contribution', targetId: created.contributionId },
    });
    ...
    await request(app.getHttpServer())
      .get(`/restaurants/${created.restaurantId}`)
      .expect(200);
    ...
    const contribution = await prisma.contribution.findUniqueOrThrow({
      where: { id: created.contributionId },
    });
  ...
  it('surfaces related reports for the same target restaurant on a contribution queue item', async () => {
    const { token: submitterToken } = await registerUser(
      'queue-related-submitter',
    );
    const { token: reporterToken } = await registerUser(
      'queue-related-reporter',
    );
    const { token: modToken } = await registerElevated(
      'queue-related-mod',
      'moderator',
    ...
    await prisma.report.deleteMany({
      where: { targetId: created.restaurantId },
    });
    await cleanupRestaurant(created.restaurantId);
  });
});
