...
npm run dev:web       # http://localhost:3001 (backend already owns :3000)
```
...
- **A `web/` page calls `notFound()` but the response is `200 OK` with 404-looking content (a "soft 404")**: this happens specifically on route segments that have a `loading.tsx` — Next's App Router wraps the page in a Suspense boundary for the skeleton, which forces the response shell to start streaming with a `200` status before the async page function reaches its `notFound()` call, and a streamed status code can't be changed after the fact. `web/src/app/quan/[slug]/` deliberately has **no** `loading.tsx` for this reason (the search page's `loading.tsx` is safe — that route never calls `notFound()`). Don't add a loading skeleton back to a route that can 404 without re-verifying the status code (`curl -o /dev/null -w '%{http_code}' <bad-slug-url>`) — a soft 404 is a real SEO regression on a site whose whole purpose is SEO.
